...
function buildVerificationPanelEmbed(useAttachment = false) {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `Welcome to **TNM**! To unlock all server channels, community chat, voice rooms, and access passes, you must link your Roblox account.\n\n` +
      `📌 **VERIFICATION STEPS:**\n` +
      `**1.** Click **"1. Join Roblox Group"** below to join the official **TNM** Roblox Group.\n` +
  ...
  if (useAttachment) {
    ...
    embed.setThumbnail('https://www.shoptnm.org/logo.png');
    embed.setImage('https://www.shoptnm.org/ticket_banner.jpg');
    embed.setFooter({ text: 'TNM Roblox Gateway • Group ID: 316559660', iconURL: 'https://www.shoptnm.org/favicon.png' });
  }
...
function buildMustJoinEmbed(profile, groupId) {
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      `Hey **${profile ? profile.displayName : 'there'}**! You must be a member of the official **TNM** Roblox Group before you can verify.\n\n` +
      `👉 **[Click Here to Join the TNM Roblox Group](${groupUrl})**\n\n` +
    ...
    .setFooter({ text: 'TNM Roblox Verification • Group Join Required', iconURL: 'https://www.shoptnm.org/favicon.png' })
    .setTimestamp();
}
...
function buildVerifyEmbed(discordUser, robloxProfile, rankResult, guild = null) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: 'TNM Roblox Verification • Community', iconURL: 'https://www.shoptnm.org/favicon.png' })
    .setTimestamp();
